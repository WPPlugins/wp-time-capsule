...
class Wptc_Plans
{
		...
		if($this->slot_dets['slot_index'] == $single_plan_info['plan_index']){
			if(!empty($single_plan_info['plan_slots'])){
				if($single_plan_info['plan_slots'] >= 200){
					...
					if($remainingSites > 0){
						$sites_left_text = '<strong>' . $remainingSites . '</strong> sites left';
					}
				}
			}
		}
	...
	private function get_formatted_price_div($single_plan_info)
	{
		...
		$price_text .= '<div class="dollar_group"><span class="dollar_wptc"><span class="kutty_dollar" style="'.$dont_show_dollar.'">$</span>' . $single_plan_info['plan_amt'] . '</span> <span class="price_interval_wptc" style="'.$dont_show_dollar.'"> ' . $single_plan_info['plan_interval'] . '</span></div>';
    ...
    public function echo_plan_box_div_wptc()
    {
        ...
        dark_debug($plan_info, "--------plan_info-----echo_plan_box_div_wptc---");
		...
		$div .= '<div class="wptc_plans_title_info_text">Select Plan</div><input type="hidden" class="selected_plan_index_wptc"/><input type="hidden" class="selected_user_token_wptc" value="'.$encoded_pwd.'"/>
