...
class WPTC_Processed_Copiedfiles extends WPTC_Processed_Base {
	...
	public function get_file_count() {
		return $this->db->get_var("SELECT COUNT(*) FROM {$this->db->base_prefix}wptc_processed_{$this->getTableName()} WHERE download_status = 'done' AND copy_status = 'true'");
	}

	public function get_file($file_name) {
		$prepared_query = $this->db->prepare(" SELECT * FROM {$this->db->base_prefix}wptc_processed_files WHERE file = %s", $file_name);
