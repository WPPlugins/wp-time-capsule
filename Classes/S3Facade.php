...
class WPTC_S3Facade {
	...
	private function is_forbidden_error($http_code) {
	...
	public function upload_file($path, $file) {
		...
		while ($i++ < self::RETRY_COUNT) {
			try {
				...
				return $this->s3_wrapper->putFile($file, remove_secret($file), $path);
			} catch (Exception $e) {
	...
	public function chunk_upload_file($path, $file, $processed_file, $starting_backup_path_time, $meta_data_backup = null) {
		...
		return $this->s3_wrapper->chunkedUpload($file, remove_secret($file), $path, true, $offest, $upload_id, $starting_backup_path_time, $s3_part_number, $s3_parts_array);
	}
	...
	public function download_file($path, $file, $revision = '', $isChunkDownload = null, $g_file_id = null) {
		...
		while ($i++ < self::RETRY_COUNT) {
			try {
				...
				dark_debug($file, "--------retrying-chunk_download_file-------");
				if ($i > self::RETRY_COUNT) {
					$base_name_file = basename($file);
	...
	public function abort_all_multipart_uploads($parent_folder) {
		...
		foreach ($running_multi_part_uploads['Uploads'] as $k => $v) {
			...
			if ($safe_to_abort) {
				dark_debug($v, "--------running_multi_part_uploads--------");
	...
	private function process_exception($e){
		...
		if ($this->is_forbidden_error($http_code)) {
			$this->unlink_account();
		...
		if ($http_code == 503) {
			return $this->return_based_on_req($http_code, $err_msg, array('too_many_requests' => $e->getMessage()));
		}
