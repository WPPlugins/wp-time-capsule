 ...
 public function fetch($method, $url, $call, array $additional = array(), $isChunkDownload = array())
	{
		...
		// dark_debug($request, '--------$request--------');
		if ($request === false) {
			dark_debug(array(), '--------Coming--------');
		...
		if ($method == 'GET' && $this->outFile) { // GET
			dark_debug(array(), '--------GET REQUEST AND DOWNOAD FILE--------');
			...
			dark_debug(array(), '--------POST REQUEST AND FILE DOWNLOAD--------');
